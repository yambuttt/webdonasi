                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 pb-12 border-b border-slate-200">
                    ...
                    <div class="md:col-span-3 space-y-3">
                        ...
                        <ul class="space-y-2 text-sm text-charcoal-lighter font-medium">
                            <li class="flex items-center gap-2">
                                ...
                                {{ App\Models\Setting::get('contact_email', 'support@pedulia.com') }}
                            </li>
                            <li class="flex items-center gap-2">
                                <svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.94.725l.548 2.2a1 1 0 01-.321.988l-1.305.98a10.582 10.582 0 004.872 4.872l.98-1.305a1 1 0 01.988-.321l2.2.548a1 1 0 01.725.94V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                {{ App\Models\Setting::get('contact_phone', '(021) 8293-1029') }}
                            </li>
                            <li class="flex items-center gap-2">
                                <svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                {{ App\Models\Setting::get('contact_address', 'Menteng, Jakarta Pusat, Indonesia') }}
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-3">
                        ...
                        <div class="flex flex-wrap gap-2.5 pt-1">
                            @php
                                $socialMediaJson = App\Models\Setting::get('social_media');
                                $socialMediaList = $socialMediaJson ? json_decode($socialMediaJson, true) : [];
                                if (empty($socialMediaList)) {
                                    $socialMediaList = [
                                        [
                                            'id' => 'facebook',
                                            'name' => 'Facebook',
                            ...
                            @foreach($socialMediaList as $social)
                                @if(isset($social['is_active']) && $social['is_active'] && !empty($social['url']))
                                    <a href="{{ $social['url'] }}" target="_blank" title="{{ $social['name'] ?? ucfirst($social['id']) }}" class="p-2 bg-slate-200 hover:bg-primary text-charcoal rounded-full transition-colors flex items-center justify-center h-8.5 w-8.5">
                                        @if(($social['icon_type'] ?? 'default') === 'default')
                                            @if(($social['icon_default'] ?? '') === 'facebook')
                                            ...
                                            @elseif(($social['icon_default'] ?? '') === 'twitter')
                                            ...
                                            @elseif(($social['icon_default'] ?? '') === 'instagram')
                                        ...
                                        @else
                                            @if($social['icon_custom_path'])
                                                <img src="{{ $social['icon_custom_path'] }}" alt="{{ $social['name'] }}" class="h-4.5 w-4.5 object-contain">
                                            @else
                                                <span class="text-[9px] font-bold text-slate-500 uppercase">{{ substr($social['name'] ?? 'SM', 0, 2) }}</span>
                                            @endif
                                        @endif
                                    </a>
                                @endif
