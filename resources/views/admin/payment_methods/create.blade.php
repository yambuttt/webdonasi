...
@section('title', 'Tambah Metode Pembayaran - Pedulia')
...
<div class="space-y-6 max-w-3xl">
    ...
    <div>
        ...
        <h1 class="text-2xl font-black text-charcoal tracking-tight">Tambah Metode Pembayaran Baru</h1>
        <p class="text-xs text-slate-400 font-semibold mt-0.5">Konfigurasi opsi pembayaran baru untuk kampanye donasi</p>
    </div>

    <!-- Form Card -->
    <form action="{{ route('admin.payment-methods.store') }}" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 md:p-8 space-y-6">
        @csrf
        ...
        <div class="space-y-2.5 border-t border-slate-50 pt-5">
            ...
            <div class="flex items-center space-x-4">
                <div class="w-20 h-14 border border-slate-200 rounded-xl overflow-hidden bg-slate-50 p-1 flex-shrink-0 flex items-center justify-center shadow-inner">
                    <img id="logo-preview" src="" alt="Preview Logo" class="w-full h-full object-contain hidden">
                    <span id="logo-placeholder" class="text-[9px] text-slate-400 font-extrabold uppercase text-center">No Logo</span>
                </div>
                <div class="space-y-1">
                    ...
                    <span class="block text-[10px] text-slate-400 font-semibold">Format: JPG, PNG, JPEG, SVG. Maks 2MB.</span>
                </div>
            </div>
        </div>
        ...
        <div id="qris-fields-container" class="space-y-2.5 border-t border-slate-50 pt-5">
            <label class="block text-xs font-bold text-charcoal">Gambar Barcode QRIS (Wajib)</label>
            <div class="flex items-center space-x-4">
                <div class="w-28 h-28 border border-slate-200 rounded-2xl overflow-hidden bg-slate-50 p-1 flex-shrink-0 flex items-center justify-center shadow-inner">
                    <img id="qris-preview" src="" alt="Preview QRIS" class="w-full h-full object-contain hidden">
                    <span id="qris-placeholder" class="text-[9px] text-slate-400 font-extrabold uppercase text-center">No Image</span>
                </div>
                <div class="space-y-1">
                    ...
                    <span class="block text-[10px] text-slate-400 font-semibold">Upload file QRIS code Anda. Maks 2MB.</span>
                </div>
            </div>
        </div>
        ...
        <div id="bank-fields-container" class="space-y-5 border-t border-slate-50 pt-5 hidden">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="space-y-2">
                    ...
                    <input type="text" name="bank_name" id="bank_name" value="{{ old('bank_name') }}" placeholder="Contoh: BANK MANDIRI, BNI" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold placeholder-slate-400 focus:outline-none focus:bg-white focus:border-charcoal focus:ring-1 focus:ring-charcoal transition-all">
                </div>

                <div class="space-y-2">
                    <label for="bank_account_number" class="block text-xs font-bold text-charcoal">Nomor Rekening (Wajib)</label>
                    <input type="text" name="bank_account_number" id="bank_account_number" value="{{ old('bank_account_number') }}" placeholder="Contoh: 123-456-7890" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold placeholder-slate-400 focus:outline-none focus:bg-white focus:border-charcoal focus:ring-1 focus:ring-charcoal transition-all">
                </div>

                <div class="space-y-2">
                    <label for="bank_account_name" class="block text-xs font-bold text-charcoal">Nama Pemilik Rekening (Wajib)</label>
                    <input type="text" name="bank_account_name" id="bank_account_name" value="{{ old('bank_account_name') }}" placeholder="Contoh: Yayasan Pedulia" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold placeholder-slate-400 focus:outline-none focus:bg-white focus:border-charcoal focus:ring-1 focus:ring-charcoal transition-all">
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-slate-100 pt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center space-x-3">
                ...
                <input type="checkbox" name="status" id="status" value="1" checked class="h-4.5 w-4.5 rounded border-slate-300 text-slate-900 focus:ring-slate-900 focus:ring-offset-0">
                <label for="status" class="text-xs font-bold text-charcoal cursor-pointer">Metode Pembayaran ini langsung aktif</label>
            </div>
