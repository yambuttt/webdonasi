...
@section('title', 'Edit Kampanye - Pedulia')
@section('header-title', 'Edit Kampanye')
@section('header-subtitle', 'Perbarui data kampanye donasi, nominal target, dan opsi nominal')
...
@section('content')
    <form id="campaign-form" action="{{ route('admin.campaigns.update', $campaign->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <select id="category" name="category" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-charcoal focus:outline-none focus:border-charcoal focus:ring-1 focus:ring-charcoal transition-all">
                                <option value="kesehatan" {{ old('category', $campaign->category) === 'kesehatan' ? 'selected' : '' }}>Medis & Kesehatan</option>
                                <option value="bencana" {{ old('category', $campaign->category) === 'bencana' ? 'selected' : '' }}>Bencana Alam</option>
            ...
            <div class="space-y-6">
                ...
                <!-- Status & Time Settings Card -->
                <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Pengaturan Kampanye</h3>
                    
                    <div class="space-y-2">
                        <label for="status" class="text-xs font-bold text-charcoal">Status Kampanye</label>
                        <select id="status" name="status" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-charcoal focus:outline-none focus:border-charcoal focus:ring-1 focus:ring-charcoal transition-all">
                            ...
                            <option value="completed" {{ old('status', $campaign->status) === 'completed' ? 'selected' : '' }}>Selesai / Terpenuhi</option>
                            <option value="draft" {{ old('status', $campaign->status) === 'draft' ? 'selected' : '' }}>Simpan Sebagai Draf</option>
                        </select>
                        @error('status')
                            <p class="text-xs text-red-500 font-bold mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" id="days_remaining" name="days_remaining" value="{{ old('days_remaining', $campaign->days_remaining) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-charcoal focus:outline-none focus:border-charcoal focus:ring-1 focus:ring-charcoal transition-all">
                ...
                <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div id="nominal-hidden-inputs">
                        @if(old('donation_options', $campaign->donation_options))
                            @foreach(old('donation_options', $campaign->donation_options) as $option)
                ...
                <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div id="thumbnail-preview-box" class="h-44 w-full bg-slate-50 border border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-400 relative overflow-hidden group cursor-pointer">
                        <img id="thumbnail-preview-img" src="{{ $campaign->thumbnail }}" class="absolute inset-0 w-full h-full object-cover {{ $campaign->thumbnail ? '' : 'hidden' }}">
                        
                        <div id="preview-placeholder" class="flex flex-col items-center justify-center {{ $campaign->thumbnail ? 'hidden' : '' }}">
                            <svg class="h-8 w-8 mb-2 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
